...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8000/api/client/token/", {
        email,
        password,
      });
      ...
      if (response.status === 200) {
        setMessage("Welcome to Hotel Gourdo!");
        // Save the tokens or do any additional logic as required
        // For example, localStorage.setItem('access_token', response.data.access);
      } else {
        setMessage("Invalid credentials. Please try again.");
      }
    } catch (error) {
